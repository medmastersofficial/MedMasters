window.MM = window.MM || {};
window.MM.fpsc_past_6 = [
  {
    "q": "Choose the word most SIMILAR in meaning to TENACIOUS.",
    "opts": [
      "Resolute",
      "Languid",
      "Vacuous",
      "Servile"
    ],
    "ans": 0,
    "exp": "TENACIOUS means holding firmly to something, whether a position, purpose, or material. RESOLUTE is its closest synonym, meaning admirably purposeful and unwavering. LANGUID (B) means weak or lacking energy, essentially opposite in connotation. VACUOUS (C) means lacking thought or intelligence. SERVILE (D) means excessively submissive, unrelated to tenacity.",
    "section": "English",
    "num": 1
  },
  {
    "q": "Choose the word most OPPOSITE in meaning to PERFUNCTORY.",
    "opts": [
      "Meticulous",
      "Cursory",
      "Desultory",
      "Negligent"
    ],
    "ans": 0,
    "exp": "PERFUNCTORY means carried out with minimal effort and little care, as a matter of routine. Its antonym is METICULOUS, meaning showing great attention to detail and taking care not to make mistakes. CURSORY (B) and DESULTORY (C) are both near synonyms of perfunctory, implying a lack of thoroughness. NEGLIGENT (D) is also a near synonym, meaning failing to take proper care.",
    "section": "English",
    "num": 2
  },
  {
    "q": "The committee chairperson was criticised for being _____ in her handling of the dispute: she remained _____ when clarity and decisiveness were most needed.",
    "opts": [
      "forthright / evasive",
      "equivocal / non-committal",
      "resolute / hesitant",
      "transparent / obscure"
    ],
    "ans": 1,
    "exp": "The sentence describes a chairperson who failed to provide clarity and decisiveness. EQUIVOCAL means using ambiguous language to avoid commitment, and NON-COMMITTAL means not expressing a definite opinion or intention. Both words reinforce the same failure: avoiding clear positions when clarity was required. FORTHRIGHT / EVASIVE (A) is internally contradictory. RESOLUTE / HESITANT (C) is also contradictory. TRANSPARENT / OBSCURE (D) is contradictory.",
    "section": "English",
    "num": 3
  },
  {
    "q": "QUARANTINE : CONTAGION :: TRIAGE : ?",
    "opts": [
      "Infection",
      "Urgency",
      "Diagnosis",
      "Surgery"
    ],
    "ans": 1,
    "exp": "QUARANTINE is the procedure used to manage and limit the spread of CONTAGION. The analogy requires what TRIAGE manages or assesses. TRIAGE is the process of sorting patients based on the severity and URGENCY of their condition to prioritise treatment. INFECTION (A) is managed by treatment, not triage. DIAGNOSIS (C) is a process, not what triage directly manages. SURGERY (D) is a type of treatment.",
    "section": "English",
    "num": 4
  },
  {
    "q": "In the sentence below, identify the portion that contains an error. 'The data [A] collected from the three [B] clinics were [C] clearly showing [D] a rising trend.'",
    "opts": [
      "were clearly showing",
      "collected from the three",
      "The data",
      "a rising trend"
    ],
    "ans": 0,
    "exp": "The error lies in 'were clearly showing.' With progressive tenses in formal reporting, the simple past is preferred: 'clearly showed' rather than 'were clearly showing.' Additionally, in formal academic and medical English, data takes a simple past tense in declarative statements about collected findings: 'the data collected clearly showed.' Options B, C, and D are grammatically and stylistically correct.",
    "section": "English",
    "num": 5
  },
  {
    "q": "PROPHYLAXIS : DISEASE :: ARBITRATION : ?",
    "opts": [
      "Conflict",
      "Punishment",
      "Legislation",
      "Judgement"
    ],
    "ans": 0,
    "exp": "PROPHYLAXIS is the preventive measure or action taken to prevent DISEASE. ARBITRATION is the process of resolving a CONFLICT through a neutral third party. The relationship is: an intervention that prevents or resolves the second term. PUNISHMENT (B) follows wrongdoing rather than preventing or resolving a dispute. LEGISLATION (C) creates law but is not what arbitration resolves. JUDGEMENT (D) is an outcome of arbitration, not what it addresses.",
    "section": "English",
    "num": 6
  },
  {
    "q": "The two departments could not agree on the budget, _____ led to weeks of delay.",
    "opts": [
      "that",
      "which",
      "who",
      "whose"
    ],
    "ans": 1,
    "exp": "'Which' introduces a non-restrictive clause referring back to the entire preceding idea ('could not agree'), a construction 'that' cannot be used for.",
    "section": "English",
    "num": 7
  },
  {
    "q": "It is imperative that the report _____ reviewed before submission.",
    "opts": [
      "is",
      "was",
      "be",
      "will be"
    ],
    "ans": 2,
    "exp": "After expressions of necessity ('it is imperative that'), the subjunctive base form 'be' is used regardless of subject or tense.",
    "section": "English",
    "num": 8
  },
  {
    "q": "Choose the correctly structured sentence:",
    "opts": [
      "Rarely he complains about his workload.",
      "Rarely does he complain about his workload.",
      "Rarely he does complain about his workload.",
      "He rarely does complain about his workload."
    ],
    "ans": 1,
    "exp": "Negative-frequency adverbs like 'rarely' at the start of a sentence require subject-auxiliary inversion: 'Rarely does he complain...'",
    "section": "English",
    "num": 9
  },
  {
    "q": "Rearrange to form a meaningful sentence: (1) the vaccine (2) distributed (3) was (4) to remote areas (5) equitably.",
    "opts": [
      "1-3-5-2-4",
      "1-2-3-5-4",
      "3-1-2-5-4",
      "1-3-2-5-4"
    ],
    "ans": 0,
    "exp": "The correct order is 'The vaccine was equitably distributed to remote areas' — subject (1), verb (3), adverb (5), participle (2), then the phrase (4).",
    "section": "English",
    "num": 10
  },
  {
    "q": "Choose the best combination: 'The clinic was newly built. It lacked basic equipment.'",
    "opts": [
      "The clinic was newly built, so it lacked basic equipment.",
      "Although the clinic was newly built, it lacked basic equipment.",
      "The clinic was newly built because it lacked basic equipment.",
      "It lacked basic equipment, so the clinic was newly built."
    ],
    "ans": 1,
    "exp": "'Although' correctly signals the contrast — despite being newly built, the clinic still lacked equipment. The other options misstate the logical relationship.",
    "section": "English",
    "num": 11
  },
  {
    "q": "Choose the synonym of PRUDENT:",
    "opts": [
      "Reckless",
      "Sensible and cautious",
      "Careless",
      "Impulsive"
    ],
    "ans": 1,
    "exp": "PRUDENT means acting with or showing care and thought for the future; sensible and cautious in practical matters.",
    "section": "English",
    "num": 12
  },
  {
    "q": "Choose the antonym of BENEVOLENT:",
    "opts": [
      "Kind",
      "Malicious",
      "Generous",
      "Charitable"
    ],
    "ans": 1,
    "exp": "BENEVOLENT means well-meaning and kindly; its opposite is MALICIOUS, meaning having or showing intention to harm.",
    "section": "English",
    "num": 13
  },
  {
    "q": "ARCHITECT : BLUEPRINT :: SURGEON : ?",
    "opts": [
      "Scalpel",
      "Operative plan",
      "Hospital",
      "Anaesthetic"
    ],
    "ans": 1,
    "exp": "An architect works from a blueprint before building; similarly, a surgeon works from an operative plan before operating. The relationship tested is 'planning document used before the main task'.",
    "section": "English",
    "num": 14
  },
  {
    "q": "The word 'INNOCUOUS' most nearly means:",
    "opts": [
      "Harmful",
      "Not harmful or offensive",
      "Extremely toxic",
      "Highly contagious"
    ],
    "ans": 1,
    "exp": "INNOCUOUS means not harmful or offensive; harmless.",
    "section": "English",
    "num": 15
  },
  {
    "q": "Identify the grammatically correct sentence:",
    "opts": [
      "Neither of the reports were accurate.",
      "Neither of the reports was accurate.",
      "Neither of report were accurate.",
      "Neither of the reports being accurate."
    ],
    "ans": 1,
    "exp": "'Neither of' takes a singular verb regardless of the plural noun that follows ('of the reports'), so 'was accurate' is correct.",
    "section": "English",
    "num": 16
  },
  {
    "q": "_____ repeated warnings, the factory continued to violate safety regulations.",
    "opts": [
      "Despite",
      "Because of",
      "So that",
      "In order to"
    ],
    "ans": 0,
    "exp": "'Despite' introduces a contrast — the factory kept violating regulations even after repeated warnings. 'Because of' would incorrectly suggest the warnings caused the violations.",
    "section": "English",
    "num": 17
  },
  {
    "q": "Choose the one word for: 'A remedy for all diseases or problems.'",
    "opts": [
      "Antidote",
      "Panacea",
      "Prophylaxis",
      "Vaccine"
    ],
    "ans": 1,
    "exp": "A PANACEA is a solution or remedy claimed to be effective for all diseases or difficulties — often used figuratively for a universal fix.",
    "section": "English",
    "num": 18
  },
  {
    "q": "By the time the inspection team arrives, the renovations _____ for over a year.",
    "opts": [
      "will continue",
      "will have been continuing",
      "continue",
      "continued"
    ],
    "ans": 1,
    "exp": "An action continuing up to a specific future point requires the future perfect continuous tense: 'will have been continuing'.",
    "section": "English",
    "num": 19
  },
  {
    "q": "Choose the correctly punctuated sentence:",
    "opts": [
      "The director, unfortunately could not attend the ceremony.",
      "The director, unfortunately, could not attend the ceremony.",
      "The director unfortunately, could not attend the ceremony.",
      "The director unfortunately could not, attend the ceremony."
    ],
    "ans": 1,
    "exp": "The parenthetical adverb 'unfortunately' placed mid-sentence must be set off by commas on both sides: 'The director, unfortunately, could not attend...'",
    "section": "English",
    "num": 20
  },
  {
    "q": "A 28-year-old man sustains a stab wound to the posterior triangle of the neck. He develops weakness of shoulder shrugging and inability to turn his head to the opposite side against resistance. Which nerve is most likely injured?",
    "opts": [
      "Phrenic nerve",
      "Accessory nerve (CN XI)",
      "Hypoglossal nerve (CN XII)",
      "Greater auricular nerve"
    ],
    "ans": 1,
    "exp": "The accessory nerve (CN XI) travels through the posterior triangle of the neck, making it the most vulnerable cranial nerve to penetrating neck injuries in this region. It supplies the sternocleidomastoid (turning head to opposite side) and trapezius (shoulder shrugging and scapular elevation). Injury produces weakness of these specific movements as described. As Snell's Clinical Anatomy notes, the accessory nerve is superficially placed in the posterior triangle, crossing it from the posterior border of the sternocleidomastoid to the anterior border of the trapezius. The phrenic nerve (A) is in the anterior triangle and causes hemidiaphragm paralysis. Hypoglossal nerve (C) supplies tongue muscles.",
    "section": "Basic Sciences",
    "num": 21
  },
  {
    "q": "A 45-year-old woman has a parotid gland tumour requiring surgical excision. During the procedure, the surgeon must identify and preserve a nerve that runs through the substance of the parotid gland. Which nerve is this?",
    "opts": [
      "Trigeminal nerve (CN V)",
      "Facial nerve (CN VII)",
      "Glossopharyngeal nerve (CN IX)",
      "Auriculotemporal nerve"
    ],
    "ans": 1,
    "exp": "The facial nerve (CN VII) exits the stylomastoid foramen and enters the posterior aspect of the parotid gland, dividing within it into its five terminal branches: temporal, zygomatic, buccal, marginal mandibular, and cervical. Parotidectomy carries the risk of facial nerve injury causing ipsilateral facial palsy. Identification and preservation of the facial nerve is the central technical challenge of this operation. The trigeminal nerve (A) provides sensory supply to the face. The glossopharyngeal nerve (C) provides parasympathetic supply to the parotid via the lesser petrosal nerve but does not traverse the gland itself. The auriculotemporal nerve (D) is a branch of CN V3 carrying postganglionic secretomotor fibres.",
    "section": "Basic Sciences",
    "num": 22
  },
  {
    "q": "A 70-year-old man undergoes a right hemicolectomy. The surgeon ligates the right colic artery. Which vessel is the right colic artery a branch of?",
    "opts": [
      "Inferior mesenteric artery",
      "Coeliac trunk",
      "Superior mesenteric artery",
      "Internal iliac artery"
    ],
    "ans": 2,
    "exp": "The superior mesenteric artery (SMA) supplies the midgut derivatives: from the distal duodenum to the proximal two thirds of the transverse colon. Its branches include the ileocolic artery, right colic artery, and middle colic artery. The right colic artery supplies the ascending colon. The inferior mesenteric artery (A) supplies the hindgut (distal transverse colon to upper rectum) via the left colic, sigmoid arteries, and superior rectal artery. The coeliac trunk (B) supplies the foregut (stomach, liver, spleen, pancreas, proximal duodenum). The internal iliac artery (D) supplies pelvic structures.",
    "section": "Basic Sciences",
    "num": 23
  },
  {
    "q": "During a thyroid surgery, the surgeon inadvertently injures a structure running in the groove between the trachea and oesophagus on the right side. The patient wakes up with hoarseness. Which structure was injured?",
    "opts": [
      "Right vagus nerve",
      "Right external laryngeal nerve",
      "Right recurrent laryngeal nerve",
      "Right superior laryngeal nerve"
    ],
    "ans": 2,
    "exp": "The recurrent laryngeal nerve (RLN) runs in the tracheo-oesophageal groove on both sides and is the structure most at risk during thyroid surgery. It supplies all intrinsic muscles of the larynx except the cricothyroid and carries sensation from the subglottic mucosa. Injury causes ipsilateral vocal cord palsy with hoarseness (unilateral injury) or stridor and respiratory compromise (bilateral injury). The right RLN loops around the right subclavian artery and is shorter than the left. The right vagus nerve (A) runs in the carotid sheath and would not be injured in this groove. The external laryngeal nerve (B) supplies cricothyroid, and injury causes subtle voice changes rather than frank hoarseness. The superior laryngeal nerve (D) supplies sensation above the vocal cords.",
    "section": "Basic Sciences",
    "num": 24
  },
  {
    "q": "A 19-year-old male is brought to the emergency department after collapsing during a football match in Lahore in July. His core temperature is 41.2°C, he is confused, and his skin is hot and dry. Which physiological mechanism has primarily failed?",
    "opts": [
      "Evaporative heat loss through sweating",
      "Radiation of heat from skin surface vessels",
      "Conductive heat transfer to the ground",
      "Reduction of metabolic heat production"
    ],
    "ans": 0,
    "exp": "Classic heat stroke is distinguished from heat exhaustion by the triad of hyperthermia above 40°C, central nervous system dysfunction, and absence of sweating (anhidrosis). The primary physiological failure is the collapse of the sweating mechanism, which is the body's most effective means of heat dissipation in hot humid conditions. Sweat evaporation normally accounts for the majority of heat loss during exercise. Radiation (B) is impaired by ambient heat but is not the primary mechanism that has failed. Conduction (C) contributes minimally. Reduction of metabolic heat production (D) is not a physiological cooling mechanism.",
    "section": "Basic Sciences",
    "num": 25
  },
  {
    "q": "A researcher is studying the Frank-Starling mechanism in an isolated heart preparation. When preload is increased by raising end-diastolic volume, which of the following best explains the resulting increase in stroke volume?",
    "opts": [
      "Increased calcium release from the sarcoplasmic reticulum due to greater myofilament stretch",
      "Activation of the sympathetic nervous system increasing heart rate",
      "Increased systemic vascular resistance reducing afterload",
      "Reduced parasympathetic tone allowing greater contractility"
    ],
    "ans": 0,
    "exp": "The Frank-Starling mechanism states that stroke volume increases as ventricular end-diastolic volume increases, within physiological limits. The cellular basis is length-dependent activation: stretching of the sarcomere increases the sensitivity of troponin C to calcium and optimises the overlap of actin and myosin filaments, enhancing cross-bridge formation and contractile force. Additionally, stretch increases calcium release from the sarcoplasmic reticulum. This is an intrinsic cardiac property independent of neural input. Sympathetic activation (B) is extrinsic and increases inotropy independent of preload. Systemic vascular resistance (C) is afterload, not preload. Parasympathetic tone (D) affects heart rate primarily, not preload-related contractility.",
    "section": "Basic Sciences",
    "num": 26
  },
  {
    "q": "During a Valsalva manoeuvre, a patient forcibly expires against a closed glottis. Which of the following correctly describes Phase II of this manoeuvre?",
    "opts": [
      "Reduced venous return, falling cardiac output, and reflex tachycardia",
      "Increased venous return, rising blood pressure, and bradycardia",
      "Sudden overshoot of blood pressure followed by reflex bradycardia",
      "Equalisation of intrathoracic and atmospheric pressure"
    ],
    "ans": 0,
    "exp": "The Valsalva manoeuvre has four phases. Phase I (onset of straining): brief rise in BP due to compression of the aorta. Phase II (sustained straining): increased intrathoracic pressure impedes venous return to the right heart, reducing cardiac output and blood pressure, triggering baroreceptor-mediated reflex tachycardia and peripheral vasoconstriction. Phase III (release): sudden drop in BP as intrathoracic pressure normalises. Phase IV (recovery): blood rushes back, cardiac output overshoots, BP rises above baseline, causing reflex bradycardia. Option B describes Phase IV. Option C also describes Phase IV. Option D is not a defined phase.",
    "section": "Basic Sciences",
    "num": 27
  },
  {
    "q": "A physiologist is studying the oxygen-haemoglobin dissociation curve. Which of the following conditions would shift the curve to the RIGHT, facilitating oxygen unloading at the tissues?",
    "opts": [
      "Increased temperature, increased PCO2, and decreased pH",
      "Decreased temperature, decreased PCO2, and increased pH",
      "Decreased 2,3-DPG levels and alkalosis",
      "Carbon monoxide poisoning"
    ],
    "ans": 0,
    "exp": "A rightward shift of the oxygen-haemoglobin dissociation curve decreases haemoglobin's affinity for oxygen, facilitating release at the tissues. This is caused by the Bohr effect: increased PCO2, decreased pH (acidosis), increased temperature, and increased 2,3-DPG. All of these are present in actively metabolising tissues. Option B describes conditions causing a leftward shift (increased affinity, decreased unloading). Option C (decreased 2,3-DPG and alkalosis) also causes a leftward shift. Carbon monoxide (D) shifts the curve left by increasing affinity of remaining haem groups for oxygen (Haldane effect).",
    "section": "Basic Sciences",
    "num": 28
  },
  {
    "q": "A 62-year-old man with heart failure is started on digoxin. Three days later he develops nausea, visual disturbances with yellow-green halos, and a heart rate of 42 beats per minute. His serum potassium is 2.9 mEq/L. Which factor most potentiates digoxin toxicity in this patient?",
    "opts": [
      "Hypokalaemia",
      "Hypernatraemia",
      "Metabolic alkalosis",
      "Hypercalcaemia"
    ],
    "ans": 0,
    "exp": "Digoxin toxicity is significantly potentiated by hypokalaemia. Digoxin inhibits the Na+/K+-ATPase pump. Potassium and digoxin compete for the same binding site on the alpha subunit of this pump. When extracellular potassium is low, digoxin binds more avidly to the pump, increasing its inhibitory effect and toxicity even at therapeutic serum digoxin concentrations. Clinically, this means that hypokalaemia (commonly from concurrent diuretic use) dramatically lowers the toxic threshold of digoxin. As Katzung's Basic and Clinical Pharmacology emphasises, serum potassium must be monitored during digoxin therapy. Hypercalcaemia (D) also increases digoxin toxicity but is less commonly the cause in heart failure patients.",
    "section": "Basic Sciences",
    "num": 29
  },
  {
    "q": "A 45-year-old woman with rheumatoid arthritis is treated with methotrexate. She is also prescribed folic acid supplementation. What is the rationale for co-prescribing folic acid with methotrexate in this context?",
    "opts": [
      "Folic acid enhances the anti-inflammatory effect of methotrexate",
      "Folic acid replenishes stores depleted by methotrexate's inhibition of dihydrofolate reductase",
      "Folic acid prevents renal clearance of methotrexate",
      "Folic acid acts as a prodrug that is converted to methotrexate intracellularly"
    ],
    "ans": 1,
    "exp": "Methotrexate inhibits dihydrofolate reductase (DHFR), preventing the conversion of dihydrofolate to tetrahydrofolate, which is required for purine and thymidylate synthesis. This leads to depletion of folate cofactors. In low-dose use for rheumatoid arthritis, co-prescribed folic acid bypasses DHFR and replenishes folate stores, reducing mucositis, nausea, hepatotoxicity, and macrocytic anaemia without significantly diminishing the anti-inflammatory effect (which occurs through other mechanisms including adenosine pathway modulation). Folic acid does not enhance anti-inflammatory effects (A), does not affect renal clearance (C), and is not a prodrug for methotrexate (D).",
    "section": "Basic Sciences",
    "num": 30
  },
  {
    "q": "A 55-year-old man with type 2 diabetes is started on metformin. He is warned about a rare but serious adverse effect. Which of the following best describes the mechanism of the most serious adverse effect of metformin?",
    "opts": [
      "Hypoglycaemia due to excessive insulin secretion",
      "Lactic acidosis due to inhibition of mitochondrial complex I and impaired lactate clearance",
      "Nephrotoxicity due to direct tubular damage",
      "Hepatotoxicity due to reactive oxygen species generation"
    ],
    "ans": 1,
    "exp": "The most serious adverse effect of metformin is lactic acidosis, which is rare but potentially fatal. Metformin inhibits mitochondrial complex I (NADH dehydrogenase) of the respiratory chain in the liver and intestinal wall, impairing oxidative phosphorylation and increasing anaerobic glycolysis. This leads to accumulation of lactate. Additionally, metformin inhibits hepatic gluconeogenesis from lactate, further impairing lactate clearance. The risk is highest with renal impairment (reduced metformin clearance). Metformin does not stimulate insulin secretion and does not cause hypoglycaemia (A) when used as monotherapy. Direct nephrotoxicity (C) and hepatotoxicity via ROS (D) are not mechanisms of metformin.",
    "section": "Basic Sciences",
    "num": 31
  },
  {
    "q": "A 28-year-old woman with epilepsy well-controlled on sodium valproate discovers she is 6 weeks pregnant. Her neurologist is most concerned about which teratogenic effect of sodium valproate?",
    "opts": [
      "Limb reduction defects",
      "Neural tube defects and neurodevelopmental delay",
      "Cardiac septal defects",
      "Cleft palate"
    ],
    "ans": 1,
    "exp": "Sodium valproate is a well-established teratogen, with the highest teratogenic risk of all antiepileptic drugs. Its major teratogenic effects include neural tube defects (spina bifida, anencephaly) due to folate antagonism, and significant neurodevelopmental delay, cognitive impairment, and autism spectrum disorder in exposed offspring. The absolute risk of major congenital malformations is approximately 10% with valproate, compared to 2-3% in the general population. Valproate is now subject to the Pregnancy Prevention Programme in many countries. Limb reduction defects (A) are associated with thalidomide. Cardiac septal defects (C) and cleft palate (D) are associated with other antiepileptics but are not the primary concern with valproate.",
    "section": "Basic Sciences",
    "num": 32
  },
  {
    "q": "A patient with tuberculosis is started on a standard four-drug regimen. Two months into treatment, liver function tests show ALT five times the upper limit of normal and the patient is symptomatic with jaundice. Which drug is most likely responsible?",
    "opts": [
      "Ethambutol",
      "Streptomycin",
      "Isoniazid",
      "Pyrazinamide"
    ],
    "ans": 2,
    "exp": "Drug-induced hepatotoxicity (DILI) is a well-recognised complication of antituberculous therapy. The most common hepatotoxic agent in the standard HRZE regimen is isoniazid, which causes hepatocellular injury through its toxic metabolite hydrazine, produced by acetylation. Hepatotoxicity risk is higher in slow acetylators and with concurrent alcohol use. Pyrazinamide (D) also causes hepatotoxicity (more often a cholestatic pattern and hyperuricaemia) but isoniazid is the most frequent and most clinically significant hepatotoxin in TB regimens. Ethambutol (A) causes optic neuritis, not hepatotoxicity. Streptomycin (B) causes ototoxicity and nephrotoxicity.",
    "section": "Basic Sciences",
    "num": 33
  },
  {
    "q": "A 35-year-old HIV-positive patient on antiretroviral therapy develops peripheral neuropathy. His regimen includes stavudine (d4T). What is the mechanism of stavudine-induced peripheral neuropathy?",
    "opts": [
      "Direct toxic effect on myelin basic protein",
      "Mitochondrial toxicity due to inhibition of mitochondrial DNA polymerase gamma",
      "Autoimmune demyelination triggered by immune reconstitution",
      "Vitamin B12 depletion caused by stavudine metabolism"
    ],
    "ans": 1,
    "exp": "Stavudine (d4T) is a nucleoside reverse transcriptase inhibitor (NRTI). NRTIs can inhibit mitochondrial DNA polymerase gamma, which is responsible for replicating mitochondrial DNA in peripheral neurons. This results in mitochondrial dysfunction and depletion in dorsal root ganglion neurons and peripheral nerves, causing the length-dependent sensorimotor peripheral neuropathy seen with stavudine, didanosine, and zalcitabine. This is also the mechanism behind lipoatrophy with thymidine analogues. Direct myelin toxicity (A) is not the mechanism. Immune reconstitution inflammatory syndrome (C) can cause neuropathy but is a different mechanism. Vitamin B12 depletion (D) is not a known effect of stavudine.",
    "section": "Basic Sciences",
    "num": 34
  },
  {
    "q": "In emergency and primary care history-taking, the 'SAMPLE' mnemonic (Signs/Symptoms, Allergies, Medications, Past history, Last oral intake, Events) is primarily used to:",
    "opts": [
      "Calculate drug dosages",
      "Rapidly and systematically gather a focused patient history",
      "Record laboratory results",
      "Classify diseases by severity"
    ],
    "ans": 1,
    "exp": "The SAMPLE history is a widely used structured framework for quickly and systematically gathering the essential elements of a patient's history in emergency and primary care settings, ensuring no critical detail is missed under time pressure.",
    "section": "Community Medicine",
    "num": 35
  },
  {
    "q": "According to Pakistan's national health planning data, the contraceptive prevalence rate has remained around what level, well below other countries in the region?",
    "opts": [
      "Around 75%",
      "Around 35%",
      "Around 90%",
      "Around 10%"
    ],
    "ans": 1,
    "exp": "Pakistan's National Health Vision 2016-2025 document notes that contraceptive prevalence has remained around 35%, far below other regional countries, contributing to a persistently high population growth rate and unmet need for family planning.",
    "section": "Community Medicine",
    "num": 36
  },
  {
    "q": "Skilled birth attendance (SBA) in Pakistan improved significantly between the late 1990s and 2015, rising from approximately 18% to what level?",
    "opts": [
      "25%",
      "40%",
      "58%",
      "95%"
    ],
    "ans": 2,
    "exp": "National health data cited in Pakistan's health policy documents show skilled birth attendance improved from about 18% in the late 1990s to approximately 58% by 2015, though a large gap to universal coverage remains.",
    "section": "Community Medicine",
    "num": 37
  },
  {
    "q": "Pakistan is reported to carry the world's how many highest burden of tuberculosis, according to WHO Global TB Report data cited in national health planning documents?",
    "opts": [
      "1st highest",
      "5th highest",
      "15th highest",
      "30th highest"
    ],
    "ans": 1,
    "exp": "Pakistan is cited in national health policy documents as having the 5th highest tuberculosis burden in the world, according to WHO Global TB Report data, making TB control a continuing national health priority.",
    "section": "Community Medicine",
    "num": 38
  },
  {
    "q": "The concept of 'vertical' immunization and disease control programmes, as distinct from integrated primary healthcare delivery, refers to:",
    "opts": [
      "Programmes delivered exclusively in tall buildings",
      "Disease-specific programmes run with their own dedicated staff and management structure, separate from general health services",
      "Programmes that only treat vertical (spinal) conditions",
      "A method of measuring population height"
    ],
    "ans": 1,
    "exp": "Vertical programmes (e.g., a dedicated polio eradication programme) are run with dedicated staff, funding, and management focused on a single disease, as distinct from horizontal, integrated delivery through general primary health services.",
    "section": "Community Medicine",
    "num": 39
  },
  {
    "q": "Pakistan's persistently low routine immunization coverage, despite the existence of a national EPI programme, illustrates a gap between:",
    "opts": [
      "Policy design (Care) and actual service delivery/uptake",
      "Cure and rehabilitation only",
      "Two different curative treatments",
      "Health financing and hospital infrastructure only"
    ],
    "ans": 0,
    "exp": "Even a well-designed preventive ('Care') policy like the EPI can fail to achieve its goals if actual service delivery, outreach, and community uptake remain weak — illustrating that 'Care' requires effective implementation, not just good policy on paper.",
    "section": "Care vs Cure",
    "num": 40
  },
  {
    "q": "A district that channels most of its health budget into building new tertiary hospitals, while routine immunization coverage in that district remains critically low, best illustrates:",
    "opts": [
      "A balanced Care-Cure approach",
      "An imbalance favouring Cure over Care",
      "An imbalance favouring Care over Cure",
      "A purely administrative issue unrelated to Care vs Cure"
    ],
    "ans": 1,
    "exp": "Prioritizing expensive curative infrastructure while basic preventive coverage (immunization) remains low is a clear real-world example of a health system leaning toward 'Cure' at the expense of 'Care'.",
    "section": "Care vs Cure",
    "num": 41
  },
  {
    "q": "Community-based diarrhoeal disease prevention through improved water and sanitation is an example of which approach?",
    "opts": [
      "Cure",
      "Care (preventive, upstream intervention)",
      "Tertiary rehabilitation",
      "Palliative care"
    ],
    "ans": 1,
    "exp": "Preventing disease at its root cause (contaminated water/poor sanitation) before illness occurs is a classic upstream 'Care' intervention, contrasted with treating diarrhoeal illness after it has already developed.",
    "section": "Care vs Cure",
    "num": 42
  },
  {
    "q": "Pakistan's population growth rate, as cited in national health policy documents, has remained around what level per annum, contributing to a rapidly growing population that strains health services?",
    "opts": [
      "0.5%",
      "1.0%",
      "1.9%",
      "4.5%"
    ],
    "ans": 2,
    "exp": "National health policy documents cite Pakistan's population growth rate at approximately 1.9% per annum, a major driver of the country's rapidly growing population and a key challenge for health service planning.",
    "section": "Health Policies of Govt",
    "num": 43
  },
  {
    "q": "Pakistan's last recorded maternal mortality ratio cited in national health planning documents (2006-07 survey) was approximately how many deaths per 100,000 live births?",
    "opts": [
      "50",
      "150",
      "276",
      "500"
    ],
    "ans": 2,
    "exp": "The National Health Vision 2016-2025 cites a maternal mortality ratio of 276 per 100,000 live births from the 2006-07 Pakistan Demographic and Health Survey, a figure that has since improved but remained a major policy concern.",
    "section": "Health Policies of Govt",
    "num": 44
  },
  {
    "q": "The 18th Constitutional Amendment, which devolved health as a subject from the federal government to Pakistan's provinces, was passed in which year?",
    "opts": [
      "1973",
      "2001",
      "2010",
      "2020"
    ],
    "ans": 2,
    "exp": "The 18th Constitutional Amendment, which devolved health (along with several other subjects) from federal to provincial jurisdiction, was passed in 2010, reshaping how health policy and services are planned and delivered nationally.",
    "section": "Health Policies of Govt",
    "num": 45
  },
  {
    "q": "Pakistan is cited in global health data as having one of the highest national prevalences of which non-communicable disease, ranking 7th highest in the world?",
    "opts": [
      "Diabetes mellitus",
      "Multiple sclerosis",
      "Haemophilia",
      "Cystic fibrosis"
    ],
    "ans": 0,
    "exp": "National health policy documents cite WHO data showing Pakistan ranks 7th highest in the world for diabetes prevalence, reflecting the rising non-communicable disease burden layered on top of Pakistan's existing communicable disease challenges.",
    "section": "Major Challenges to Healthcare",
    "num": 46
  },
  {
    "q": "High rates of hypertension in Pakistan's adult population, cited as roughly 1 in 4 adults over 18, pose a major challenge primarily because:",
    "opts": [
      "Hypertension has no long-term health consequences",
      "It significantly raises the risk of stroke, heart disease, and kidney disease across a large share of the adult population",
      "It only affects the elderly",
      "It is fully preventable through vaccination"
    ],
    "ans": 1,
    "exp": "With roughly one in four adults over 18 affected, uncontrolled hypertension substantially raises population-wide risk of stroke, ischaemic heart disease, and chronic kidney disease, making it a major driver of non-communicable disease burden in Pakistan.",
    "section": "Major Challenges to Healthcare",
    "num": 47
  },
  {
    "q": "Persistently high smoking prevalence among men in Pakistan, cited at around 38% in national health data, is considered a major challenge chiefly because it:",
    "opts": [
      "Has no impact on national disease burden",
      "Substantially drives the burden of COPD, cardiovascular disease, and cancers, straining the health system",
      "Only affects smokers' personal finances",
      "Is already fully addressed through existing policy"
    ],
    "ans": 1,
    "exp": "High male smoking prevalence is a major contributor to Pakistan's growing burden of chronic obstructive pulmonary disease, cardiovascular disease, and various cancers, adding significant long-term pressure on an already stretched health system.",
    "section": "Major Challenges to Healthcare",
    "num": 48
  },
  {
    "q": "A 58-year-old man presents to a government hospital in Karachi with progressive exertional dyspnoea, bilateral pitting oedema up to the knees, raised JVP, and a third heart sound on auscultation. Chest X-ray shows cardiomegaly with upper lobe venous diversion. What is the most likely diagnosis?",
    "opts": [
      "Constrictive pericarditis",
      "Congestive cardiac failure",
      "Nephrotic syndrome",
      "Liver cirrhosis with portal hypertension"
    ],
    "ans": 1,
    "exp": "The combination of exertional dyspnoea, bilateral pitting oedema, raised JVP, third heart sound (S3 gallop), and cardiomegaly with upper lobe venous diversion on chest X-ray is the classic presentation of congestive cardiac failure (CCF). The raised JVP reflects elevated right-sided filling pressures. The S3 gallop arises from rapid ventricular filling into a dilated, non-compliant ventricle. Upper lobe blood diversion on CXR indicates elevated pulmonary venous pressure causing redistribution from lower to upper lobes. Constrictive pericarditis (A) can mimic CCF but typically shows a normal heart size with pericardial calcification. Nephrotic syndrome (C) causes oedema without raised JVP or cardiac signs. Cirrhosis (D) causes ascites predominantly rather than this cardiac picture.",
    "section": "Medicine",
    "num": 49
  },
  {
    "q": "A 40-year-old woman presents with a butterfly-shaped rash across both cheeks sparing the nasolabial folds, joint pains, and fatigue. Investigations reveal anaemia, lymphopenia, and a strongly positive ANA (1:640). Which additional test would be most specific for her likely diagnosis?",
    "opts": [
      "Anti-CCP antibody",
      "Anti-dsDNA antibody",
      "Rheumatoid factor",
      "Anti-Ro antibody"
    ],
    "ans": 1,
    "exp": "The clinical picture of malar (butterfly) rash sparing the nasolabial folds, arthralgia, haematological involvement, and strongly positive ANA is characteristic of Systemic Lupus Erythematosus (SLE). Anti-double-stranded DNA (anti-dsDNA) antibodies are highly specific for SLE (specificity approximately 97%) and correlate with disease activity and lupus nephritis. Anti-CCP (A) is specific for rheumatoid arthritis. Rheumatoid factor (C) is positive in many autoimmune conditions and lacks specificity for SLE. Anti-Ro (SSA) (D) is associated with Sjogren's syndrome, neonatal lupus, and subacute cutaneous lupus, and is less specific for SLE than anti-dsDNA. Davidson's Medicine identifies anti-dsDNA as the most diagnostically specific antibody for SLE.",
    "section": "Medicine",
    "num": 50
  },
  {
    "q": "A 65-year-old man with a 40-pack-year smoking history is found to have a fasting blood glucose of 7.4 mmol/L on two separate occasions. He is asymptomatic. Which of the following is the most appropriate first-line management?",
    "opts": [
      "Commence insulin therapy immediately",
      "Lifestyle modification with dietary counselling and exercise, and re-check HbA1c in 3 months",
      "Start metformin immediately without any lifestyle advice",
      "Refer for bariatric surgery"
    ],
    "ans": 1,
    "exp": "Two fasting glucose readings of 7.0 mmol/L or above confirm a diagnosis of type 2 diabetes mellitus. In an asymptomatic patient with newly diagnosed type 2 DM, lifestyle intervention (dietary modification, regular physical activity, weight management) is the essential foundation of management and should always accompany or precede pharmacological therapy unless HbA1c is significantly elevated. Metformin is the first-line pharmacological agent when lifestyle measures alone are insufficient or HbA1c remains above target. Insulin (A) is not first-line in asymptomatic type 2 DM. Starting metformin without lifestyle advice (C) is incomplete practice. Bariatric surgery (D) may be considered in selected obese patients but is not the immediate first-line step.",
    "section": "Medicine",
    "num": 51
  },
  {
    "q": "A 50-year-old woman is admitted with confusion, fever of 38.9°C, and neck stiffness. CSF analysis shows: opening pressure 280 mmH2O, turbid appearance, glucose 1.8 mmol/L (serum glucose 6.2 mmol/L), protein 1.8 g/L, and neutrophil count 2,400 cells/µL. What is the most likely causative organism in an adult in Pakistan?",
    "opts": [
      "Mycobacterium tuberculosis",
      "Neisseria meningitidis",
      "Streptococcus pneumoniae",
      "Listeria monocytogenes"
    ],
    "ans": 2,
    "exp": "The CSF findings of turbid fluid, markedly elevated neutrophils (2,400 cells/µL), low CSF glucose (CSF:serum ratio less than 0.4), and high protein confirm acute bacterial meningitis. In adults, Streptococcus pneumoniae is the most common cause of community-acquired bacterial meningitis, particularly in those over 50 years. This pattern holds in Pakistan as per epidemiological data. Tuberculous meningitis (A) typically shows a lymphocytic pleocytosis with very low glucose and high protein, developing over weeks. Neisseria meningitidis (B) is more common in young adults and children and is associated with a petechial/purpuric rash. Listeria (D) affects neonates, elderly, and immunocompromised patients and has lymphocytic predominance.",
    "section": "Medicine",
    "num": 52
  },
  {
    "q": "A 72-year-old man presents with a 3-month history of progressive difficulty initiating urination, poor stream, terminal dribbling, and two episodes of nocturia. Digital rectal examination reveals a smoothly enlarged, non-tender, rubbery prostate. PSA is 3.8 ng/mL. What is the most likely diagnosis?",
    "opts": [
      "Carcinoma of the prostate",
      "Prostatitis",
      "Benign prostatic hyperplasia",
      "Bladder neck contracture"
    ],
    "ans": 2,
    "exp": "The clinical presentation of progressive lower urinary tract symptoms (LUTS) including hesitancy, poor stream, terminal dribbling, and nocturia in a 72-year-old man, with smooth rubbery enlargement on DRE and a mildly elevated PSA (below 4 ng/mL is within age-adjusted normal range), is characteristic of benign prostatic hyperplasia (BPH). BPH is hyperplasia of the transitional zone of the prostate, causing bladder outlet obstruction. Carcinoma (A) typically presents with a hard, nodular, irregular prostate on DRE and PSA often above 10 ng/mL; however PSA alone cannot exclude malignancy and biopsy may be warranted. Prostatitis (B) presents acutely with fever, dysuria, and a tender prostate. Bladder neck contracture (D) is a complication of surgery or infection.",
    "section": "Medicine",
    "num": 53
  },
  {
    "q": "A 35-year-old man presents to a rural health centre in Sindh with 3 weeks of fever, dry cough, hepatosplenomegaly, and profound weight loss. Bone marrow biopsy reveals intracellular amastigotes within macrophages. Which vector transmits this disease?",
    "opts": [
      "Anopheles mosquito",
      "Culex mosquito",
      "Phlebotomus sandfly",
      "Ixodes tick"
    ],
    "ans": 2,
    "exp": "The clinical picture of prolonged fever, hepatosplenomegaly, weight loss, and intracellular amastigotes (Leishman-Donovan bodies) within macrophages on bone marrow biopsy is diagnostic of visceral leishmaniasis (kala-azar), caused by Leishmania donovani. In Pakistan and South Asia, the vector is the Phlebotomus sandfly (specifically Phlebotomus argentipes). Kala-azar is endemic in parts of Sindh and Punjab. Anopheles mosquito (A) transmits malaria. Culex mosquito (B) transmits filariasis and Japanese encephalitis. Ixodes tick (D) transmits Lyme disease and tick-borne encephalitis.",
    "section": "Medicine",
    "num": 54
  },
  {
    "q": "A 55-year-old hypertensive woman presents with sudden-onset severe headache described as 'the worst headache of my life,' followed by neck stiffness and photophobia. CT head is normal. What is the next most appropriate investigation?",
    "opts": [
      "MRI brain with gadolinium",
      "Lumbar puncture for CSF xanthochromia",
      "Cerebral angiography",
      "EEG"
    ],
    "ans": 1,
    "exp": "A thunderclap headache (sudden-onset, maximum severity at onset, 'worst headache of life') with meningism (neck stiffness, photophobia) in the absence of focal neurological signs is the classic presentation of subarachnoid haemorrhage (SAH). A normal CT does not exclude SAH: CT sensitivity is approximately 98% within 6 hours but falls to below 90% by 24 hours. When CT is negative, lumbar puncture is mandatory and the finding of xanthochromia (yellow discolouration of CSF due to haemoglobin breakdown products) or uniform blood that does not clear confirms SAH. As Davidson's Medicine emphasises, LP should be performed at least 12 hours after headache onset to allow time for xanthochromia to develop.",
    "section": "Medicine",
    "num": 55
  },
  {
    "q": "A 55-year-old man is brought to the emergency department with sudden facial droop, arm weakness, and slurred speech. Which structured assessment tool is used to rapidly screen for stroke in this setting?",
    "opts": [
      "APGAR score",
      "FAST (Face, Arm, Speech, Time)",
      "Glasgow Coma Scale only",
      "SAMPLE history"
    ],
    "ans": 1,
    "exp": "The FAST assessment (Face drooping, Arm weakness, Speech difficulty, Time to call emergency services) is a widely used rapid screening tool for identifying likely stroke and prompting urgent action, since early treatment significantly improves outcomes.",
    "section": "Medicine",
    "num": 56
  },
  {
    "q": "A patient with familial hypercholesterolaemia is already on a maximum-tolerated statin dose but has not reached LDL targets. Which additional drug acts by inhibiting intestinal absorption of cholesterol?",
    "opts": [
      "Ezetimibe",
      "Nicotinic acid",
      "Gemfibrozil",
      "Colestyramine"
    ],
    "ans": 0,
    "exp": "Ezetimibe lowers LDL cholesterol by selectively inhibiting cholesterol absorption at the brush border of the small intestine, making it a useful add-on therapy when statins alone do not achieve target LDL levels.",
    "section": "Medicine",
    "num": 57
  },
  {
    "q": "During adult cardiopulmonary resuscitation (CPR) by a single rescuer, what is the recommended ratio of chest compressions to rescue breaths?",
    "opts": [
      "15:2",
      "30:2",
      "5:1",
      "10:1"
    ],
    "ans": 1,
    "exp": "Current resuscitation guidelines recommend a compression-to-ventilation ratio of 30:2 for adult CPR performed by a single rescuer, prioritizing high-quality, largely uninterrupted chest compressions to maintain circulation.",
    "section": "Medicine",
    "num": 58
  },
  {
    "q": "A 25-year-old motorcyclist is brought to the emergency department after a road traffic accident. He has guarding and rigidity across the entire abdomen. His blood pressure is 88/60 mmHg and heart rate is 128/min. FAST ultrasound shows free fluid in Morrison's pouch and the pelvis. What is the most appropriate immediate management?",
    "opts": [
      "CT abdomen with contrast to identify the source of bleeding",
      "Emergency exploratory laparotomy",
      "IV fluid resuscitation until BP normalises followed by observation",
      "Diagnostic peritoneal lavage"
    ],
    "ans": 1,
    "exp": "This patient has haemodynamic instability (hypotension and tachycardia) with peritonism and FAST-confirmed intra-abdominal free fluid, indicating significant intra-abdominal haemorrhage. In a haemodynamically unstable patient with blunt abdominal trauma and positive FAST, the immediate management is emergency exploratory laparotomy. CT (A) requires a haemodynamically stable patient; transporting an unstable patient to CT is dangerous and wastes critical time. Fluid resuscitation alone (C) is inappropriate as the bleeding source requires surgical control; permissive hypotension targeting SBP 80-90 mmHg is preferred over aggressive resuscitation in penetrating trauma. DPL (D) has largely been superseded by FAST and is not the most appropriate step here.",
    "section": "Surgery",
    "num": 59
  },
  {
    "q": "A 45-year-old woman presents with colicky right upper quadrant pain radiating to the right shoulder tip, occurring after fatty meals, and associated with nausea and vomiting. Ultrasound confirms multiple gallstones in the gallbladder with no biliary dilation. Her LFTs and amylase are normal. What is the most appropriate management?",
    "opts": [
      "Conservative management with dietary fat restriction alone",
      "Endoscopic retrograde cholangiopancreatography (ERCP)",
      "Laparoscopic cholecystectomy",
      "Percutaneous cholecystostomy"
    ],
    "ans": 2,
    "exp": "Symptomatic cholelithiasis presenting with biliary colic is a clear indication for definitive surgical management. Laparoscopic cholecystectomy (LC) is the gold standard treatment and the procedure of choice for symptomatic gallstones without complications. It is associated with shorter hospital stay, less postoperative pain, and faster recovery compared to open surgery. Conservative management (A) may provide temporary symptom relief but does not address the underlying pathology and recurrence is near universal. ERCP (B) is indicated for choledocholithiasis (common bile duct stones), which is excluded here by normal LFTs and no biliary dilation. Percutaneous cholecystostomy (D) is reserved for acute cholecystitis in high-risk surgical patients.",
    "section": "Surgery",
    "num": 60
  },
  {
    "q": "A 60-year-old man presents with a painless swelling in the right groin. On examination, the swelling lies above and medial to the pubic tubercle, disappears on lying down, and a cough impulse is present. Which type of hernia is this?",
    "opts": [
      "Femoral hernia",
      "Direct inguinal hernia",
      "Indirect inguinal hernia",
      "Obturator hernia"
    ],
    "ans": 2,
    "exp": "The key anatomical landmark for distinguishing inguinal from femoral hernias is the pubic tubercle. Inguinal hernias lie above and medial to the pubic tubercle, while femoral hernias lie below and lateral to it. Among inguinal hernias, indirect hernias pass through the deep inguinal ring (lateral to the inferior epigastric vessels) and travel down the inguinal canal, making them reducible, often with a cough impulse. In a 60-year-old man presenting with a reducible groin swelling above the pubic tubercle, indirect inguinal hernia is the most common type overall. Bailey and Love notes that indirect inguinal hernias are the most common hernias in both sexes and all ages. Direct inguinal hernias (B) bulge through the posterior wall of the inguinal canal medial to the inferior epigastric vessels and are more common in elderly men.",
    "section": "Surgery",
    "num": 61
  },
  {
    "q": "A 70-year-old man with a long history of GORD presents with progressive dysphagia initially to solids then liquids, weight loss of 8 kg over 3 months, and iron deficiency anaemia. What is the most likely diagnosis?",
    "opts": [
      "Achalasia",
      "Oesophageal carcinoma",
      "Peptic stricture",
      "Pharyngeal pouch"
    ],
    "ans": 1,
    "exp": "Progressive dysphagia initially to solids then to liquids (indicating a mechanical, progressive obstruction rather than a neuromuscular cause), significant weight loss, iron deficiency anaemia, and a background of chronic GORD (a risk factor for Barrett's oesophagus and adenocarcinoma) in a 70-year-old man is the classic presentation of oesophageal carcinoma. Adenocarcinoma of the lower oesophagus is strongly associated with chronic GORD and Barrett's oesophagus. Achalasia (A) causes dysphagia to both solids and liquids from the outset due to failure of lower oesophageal sphincter relaxation. Peptic stricture (C) from chronic GORD is possible but tends not to cause this degree of weight loss. Pharyngeal pouch (D) causes regurgitation of undigested food, halitosis, and is proximal.",
    "section": "Surgery",
    "num": 62
  },
  {
    "q": "A 55-year-old man presents with per-rectal bleeding, change in bowel habit to looser stools for 4 months, and a palpable left iliac fossa mass. Colonoscopy reveals an ulcerating lesion in the sigmoid colon. Biopsy confirms adenocarcinoma. CT staging shows no distant metastases. What is the recommended treatment?",
    "opts": [
      "Palliative chemotherapy alone",
      "Radiotherapy alone",
      "Surgical resection with curative intent",
      "Endoscopic mucosal resection"
    ],
    "ans": 2,
    "exp": "For colorectal adenocarcinoma without distant metastases (non-metastatic disease), surgical resection with curative intent is the treatment of choice. For sigmoid colon cancer, a sigmoid colectomy with adequate lymph node clearance (at least 12 nodes) is performed. Adjuvant chemotherapy (FOLFOX or CAPOX regimen) is added for stage III (node-positive) disease. Palliative chemotherapy alone (A) is reserved for metastatic disease where cure is not possible. Radiotherapy (B) is primarily used for rectal cancer (neoadjuvant chemoradiotherapy) rather than sigmoid colon cancer. Endoscopic mucosal resection (D) is applicable only for early, superficial lesions without invasion.",
    "section": "Surgery",
    "num": 63
  },
  {
    "q": "A 30-year-old man develops a painful perianal swelling over 3 days with fever. On examination, there is a tender, fluctuant swelling at the 6 o'clock position adjacent to the anal verge. What is the most appropriate management?",
    "opts": [
      "Oral antibiotics and sitz baths",
      "Incision and drainage under anaesthesia",
      "IV antibiotics and observation",
      "MRI pelvis to characterise the abscess before any intervention"
    ],
    "ans": 1,
    "exp": "A perianal abscess presenting with pain, fever, and a fluctuant tender swelling requires prompt surgical drainage. The definitive treatment is incision and drainage (I&D) under appropriate anaesthesia. Antibiotics alone (A or C) are inadequate for a formed abscess, as they do not penetrate the abscess cavity and cannot resolve the collection. Delayed treatment risks spread of infection into the ischiorectal fossa or deeper spaces. MRI (D) is useful for complex or high-level abscesses suspected of involving multiple spaces or fistula-in-ano, but an obvious superficial perianal abscess should not have treatment delayed for imaging. Bailey and Love's Short Practice of Surgery emphasises that all perianal abscesses require surgical drainage and that spontaneous resolution is rare.",
    "section": "Surgery",
    "num": 64
  },
  {
    "q": "A 50-year-old woman presents with right calf pain, swelling, and warmth following a long-haul flight from Karachi to London. Duplex ultrasound confirms a right popliteal deep vein thrombosis. She has no contraindications to anticoagulation. What is the most appropriate management?",
    "opts": [
      "Aspirin 300 mg daily for 6 weeks",
      "Compression stockings only",
      "Therapeutic anticoagulation with a direct oral anticoagulant (DOAC)",
      "Inferior vena cava filter insertion"
    ],
    "ans": 2,
    "exp": "Confirmed proximal DVT (popliteal and above) requires therapeutic anticoagulation to prevent propagation and pulmonary embolism. Current guidelines recommend DOACs (rivaroxaban, apixaban) as first-line therapy for DVT in patients without contraindications, due to their efficacy, convenience (oral dosing), and predictable pharmacokinetics without need for routine monitoring. Aspirin (A) has insufficient anticoagulant effect for DVT treatment. Compression stockings (B) are adjuncts that reduce post-thrombotic syndrome but are not definitive treatment alone. IVC filter (D) is reserved for patients with contraindications to anticoagulation or recurrent PE despite adequate anticoagulation. Duration of anticoagulation for a provoked DVT (after a flight) is typically 3 months.",
    "section": "Surgery",
    "num": 65
  },
  {
    "q": "A 40-year-old man presents with a painless swelling above the pubic tubercle that appears on coughing and disappears on lying down. Where is the deep inguinal ring located relative to the inferior epigastric vessels?",
    "opts": [
      "Lateral to the inferior epigastric vessels",
      "Medial to the inferior epigastric vessels",
      "Directly overlying the inferior epigastric vessels",
      "Unrelated to the inferior epigastric vessels"
    ],
    "ans": 0,
    "exp": "The deep inguinal ring lies lateral to the inferior epigastric vessels, a key anatomical landmark used to distinguish indirect inguinal hernias (which pass through the deep ring, lateral to the vessels) from direct hernias (which bulge medial to the vessels).",
    "section": "Surgery",
    "num": 66
  },
  {
    "q": "A 60-year-old man presents with sudden onset of severe, tearing back pain and a pulsatile abdominal mass that has become tender. What is the most likely diagnosis?",
    "opts": [
      "Stable abdominal aortic aneurysm",
      "Ruptured or rapidly expanding abdominal aortic aneurysm",
      "Renal colic",
      "Acute pancreatitis"
    ],
    "ans": 1,
    "exp": "A previously stable pulsatile abdominal mass that becomes suddenly tender with severe tearing pain suggests rupture or rapid expansion of an abdominal aortic aneurysm — a surgical emergency requiring immediate intervention.",
    "section": "Surgery",
    "num": 67
  },
  {
    "q": "A 50-year-old woman undergoes a mastectomy with axillary lymph node dissection for breast cancer. Postoperatively, she develops arm swelling. Which structure's disruption is most responsible for this complication?",
    "opts": [
      "Axillary artery",
      "Lymphatic drainage channels of the axilla",
      "Long thoracic nerve",
      "Brachial plexus"
    ],
    "ans": 1,
    "exp": "Disruption of the axillary lymphatic drainage channels during axillary lymph node dissection commonly leads to lymphoedema of the arm, a well-recognized complication of breast cancer surgery involving axillary clearance.",
    "section": "Surgery",
    "num": 68
  },
  {
    "q": "A 26-year-old primigravida at 32 weeks gestation presents with a blood pressure of 158/104 mmHg on two readings 4 hours apart, 3+ proteinuria on dipstick, and a severe headache. She has no prior history of hypertension. What is the most likely diagnosis?",
    "opts": [
      "Gestational hypertension",
      "Chronic hypertension",
      "Pre-eclampsia with severe features",
      "HELLP syndrome"
    ],
    "ans": 2,
    "exp": "Pre-eclampsia is defined as new-onset hypertension (BP 140/90 mmHg or above on two occasions 4 hours apart) after 20 weeks gestation in a previously normotensive woman, accompanied by proteinuria or end-organ involvement. Severe features include BP 160/110 mmHg or above, severe headache, visual disturbance, epigastric pain, or markers of end-organ dysfunction. This patient has BP 158/104 mmHg (borderline severe), significant proteinuria, and severe headache at 32 weeks in a primigravida, confirming pre-eclampsia with severe features. Gestational hypertension (A) is hypertension without proteinuria or end-organ involvement. Chronic hypertension (B) pre-dates 20 weeks gestation. HELLP syndrome (D) is a complication of severe pre-eclampsia featuring Haemolysis, Elevated Liver enzymes, Low Platelets.",
    "section": "O&G",
    "num": 69
  },
  {
    "q": "A 32-year-old woman presents to antenatal clinic at 28 weeks. Her random blood glucose is 9.8 mmol/L. A subsequent 75g OGTT shows fasting glucose of 5.4 mmol/L and 2-hour glucose of 8.9 mmol/L. How should she be managed?",
    "opts": [
      "No action required as values are within normal limits",
      "Dietary modification and blood glucose monitoring; start insulin if targets not met",
      "Immediate caesarean section",
      "Start oral metformin as first-line pharmacological treatment"
    ],
    "ans": 1,
    "exp": "WHO diagnostic criteria for gestational diabetes mellitus (GDM) using 75g OGTT: fasting glucose 5.1 mmol/L or above, 1-hour glucose 10.0 mmol/L or above, or 2-hour glucose 8.5 mmol/L or above. This patient's 2-hour value of 8.9 mmol/L meets the criterion for GDM. Initial management is dietary modification (low glycaemic index diet, carbohydrate monitoring) combined with self-monitoring of blood glucose. If glucose targets (fasting below 5.3 mmol/L, 1-hour post-meal below 7.8 mmol/L) are not achieved within 1-2 weeks, insulin is started as the pharmacological treatment of choice in pregnancy. Metformin (D) may be used as an adjunct but insulin is the gold standard in pregnancy due to its safety profile and efficacy.",
    "section": "O&G",
    "num": 70
  },
  {
    "q": "A 38-year-old multigravida (G4 P3) delivers a healthy baby at term. Immediately after delivery of the placenta, she develops profuse per-vaginal bleeding. The uterus feels soft and 'boggy' on abdominal palpation. What is the most likely cause of this haemorrhage?",
    "opts": [
      "Retained placental fragments",
      "Uterine atony",
      "Cervical laceration",
      "Coagulopathy"
    ],
    "ans": 1,
    "exp": "Primary postpartum haemorrhage (PPH) is defined as blood loss of 500 mL or more within 24 hours of vaginal delivery. The four Ts mnemonic covers the causes: Tone (uterine atony), Trauma (lacerations), Tissue (retained placenta/membranes), Thrombin (coagulopathy). Uterine atony (soft, boggy uterus that fails to contract) is by far the most common cause, accounting for approximately 70-80% of all PPH cases. Risk factors include multiparity, prolonged labour, overdistension of the uterus, and oxytocin augmentation. Management begins with bimanual uterine compression and uterotonic agents (oxytocin, ergometrine, misoprostol). As outlined in Ten Teachers, this pattern of a soft uterus immediately post-delivery is pathognomonic for atony.",
    "section": "O&G",
    "num": 71
  },
  {
    "q": "A 28-year-old woman presents with 10 weeks of amenorrhoea, lower abdominal pain, and light vaginal bleeding. Urine pregnancy test is positive. Transvaginal ultrasound shows no intrauterine gestational sac and free fluid in the pouch of Douglas. Her blood pressure is 96/64 mmHg and heart rate is 116/min. What is the most appropriate immediate management?",
    "opts": [
      "Serum beta-hCG monitoring every 48 hours",
      "Methotrexate injection for medical management",
      "Emergency surgical intervention",
      "Repeat ultrasound in one week"
    ],
    "ans": 2,
    "exp": "This is a ruptured ectopic pregnancy. The triad of amenorrhoea, vaginal bleeding, and lower abdominal pain with a positive pregnancy test and no intrauterine sac on ultrasound, combined with haemodynamic instability (hypotension, tachycardia) and free fluid (haemoperitoneum) in the pouch of Douglas, indicates a surgical emergency. Emergency laparotomy (or laparoscopy in stable hands) is required immediately. Methotrexate (B) is used for unruptured, haemodynamically stable ectopic pregnancies meeting specific criteria (small size, low beta-hCG, no cardiac activity). Watchful waiting (A or D) is completely inappropriate in an unstable ruptured ectopic. This is a life-threatening emergency.",
    "section": "O&G",
    "num": 72
  },
  {
    "q": "A 25-year-old woman presents with profuse, frothy, yellow-green vaginal discharge with a fishy odour and vulval pruritus. Wet mount microscopy reveals motile, pear-shaped, flagellated organisms. What is the first-line treatment?",
    "opts": [
      "Fluconazole 150 mg single oral dose",
      "Metronidazole 400 mg twice daily for 5-7 days",
      "Azithromycin 1 g single dose",
      "Clindamycin vaginal cream"
    ],
    "ans": 1,
    "exp": "The clinical features of profuse frothy yellow-green vaginal discharge, vulval pruritus, and the characteristic motile flagellated pear-shaped organisms on wet mount microscopy are diagnostic of Trichomonas vaginalis infection, a sexually transmitted protozoal infection. The first-line treatment is metronidazole, which targets anaerobic and protozoal organisms. The regimen is either 400-500 mg twice daily for 5-7 days or a single 2 g dose. Partner notification and treatment are essential as it is sexually transmitted. Fluconazole (A) is used for Candida vulvovaginitis (thick white curd-like discharge). Azithromycin (C) treats Chlamydia and Mycoplasma. Clindamycin (D) treats bacterial vaginosis.",
    "section": "O&G",
    "num": 73
  },
  {
    "q": "A 45-year-old woman presents with secondary amenorrhoea for 8 months, hot flushes, night sweats, and vaginal dryness. FSH is 68 IU/L and LH is 52 IU/L. Serum oestradiol is low. What is the most appropriate management for her vasomotor symptoms if she has no contraindications?",
    "opts": [
      "Combined oral contraceptive pill",
      "Systemic hormone replacement therapy (HRT)",
      "Clomiphene citrate",
      "Selective serotonin reuptake inhibitor (SSRI) alone as first-line"
    ],
    "ans": 1,
    "exp": "The clinical picture of secondary amenorrhoea, vasomotor symptoms (hot flushes, night sweats), vaginal dryness, elevated FSH above 40 IU/L, and low oestradiol in a 45-year-old woman confirms premature ovarian insufficiency (POI)/menopause. Systemic hormone replacement therapy (HRT) — oestrogen with progestogen in women with an intact uterus — is the most effective treatment for vasomotor symptoms and also protects against the consequences of oestrogen deficiency including osteoporosis and cardiovascular disease. The COCP (A) could be used but is not the preferred HRT preparation for menopause management. Clomiphene (C) is an ovulation induction agent used in subfertility. SSRIs (D) may provide some benefit for vasomotor symptoms but are second-line to HRT.",
    "section": "O&G",
    "num": 74
  },
  {
    "q": "A 30-year-old woman presents with severe dysmenorrhoea, deep dyspareunia, and subfertility. Pelvic examination reveals nodularity along the uterosacral ligaments and a fixed, retroverted uterus. What is the most likely diagnosis?",
    "opts": [
      "Uterine fibroids",
      "Pelvic inflammatory disease",
      "Endometriosis",
      "Adenomyosis"
    ],
    "ans": 2,
    "exp": "The triad of severe dysmenorrhoea, deep dyspareunia, and subfertility combined with clinical findings of uterosacral ligament nodularity and a fixed retroverted uterus is the hallmark presentation of endometriosis. Endometriosis is the ectopic presence of endometrial glands and stroma outside the uterus. Uterosacral ligament involvement with nodularity is particularly specific to endometriosis. Uterine fibroids (A) typically cause menorrhagia and pressure symptoms; the uterus is enlarged and irregular but usually mobile. PID (B) presents acutely with fever, cervical excitation, and adnexal tenderness following sexually transmitted infection. Adenomyosis (D) causes a uniformly enlarged, tender, boggy uterus with severe dysmenorrhoea but not uterosacral nodularity.",
    "section": "O&G",
    "num": 75
  },
  {
    "q": "A 4-year-old child is brought to the ENT clinic with a 6-month history of bilateral hearing loss, snoring, and recurrent mouth breathing. Otoscopy shows bilateral dull, retracted tympanic membranes with no light reflex. Tympanometry reveals bilateral type B (flat) curves. What is the most likely diagnosis?",
    "opts": [
      "Acute otitis media",
      "Otitis media with effusion (glue ear)",
      "Chronic suppurative otitis media",
      "Sensorineural hearing loss"
    ],
    "ans": 1,
    "exp": "The combination of bilateral conductive hearing loss, dull retracted tympanic membranes, absent light reflex, and flat (type B) tympanograms in a child with mouth breathing and snoring is the classic presentation of otitis media with effusion (OME), commonly known as glue ear. Type B tympanogram indicates no peak pressure, consistent with fluid in the middle ear causing reduced tympanic membrane mobility. OME is the most common cause of acquired hearing loss in children in Pakistan and worldwide. Associated adenoid hypertrophy contributes to the mouth breathing and snoring. Acute otitis media (A) presents with pain, fever, and a bulging red tympanic membrane. CSOM (C) involves chronic otorrhoea through a perforation. Sensorineural loss (D) shows a normal tympanogram.",
    "section": "ENT",
    "num": 76
  },
  {
    "q": "A 35-year-old man presents with sudden onset severe vertigo, nausea, and vomiting lasting 48 hours. He reports no hearing loss and no tinnitus. Examination reveals horizontal nystagmus beating away from the affected side. The Dix-Hallpike test is negative. What is the most likely diagnosis?",
    "opts": [
      "Benign paroxysmal positional vertigo (BPPV)",
      "Vestibular neuritis",
      "Meniere's disease",
      "Acoustic neuroma"
    ],
    "ans": 1,
    "exp": "Vestibular neuritis (vestibular neuronitis) presents with sudden-onset severe vertigo lasting hours to days, nausea and vomiting, horizontal nystagmus beating away from the affected (lesioned) side, and crucially no hearing loss or tinnitus, distinguishing it from Meniere's disease. The Dix-Hallpike test is negative, ruling out BPPV. The cause is thought to be viral inflammation of the vestibular nerve. BPPV (A) is characterised by brief episodes of positional vertigo triggered by head movement and a positive Dix-Hallpike test with geotropic rotatory nystagmus. Meniere's disease (C) presents with the triad of episodic vertigo, fluctuating sensorineural hearing loss, and tinnitus. Acoustic neuroma (D) has an insidious onset with unilateral hearing loss.",
    "section": "ENT",
    "num": 77
  },
  {
    "q": "A 10-year-old boy presents with a three-day history of right-sided otalgia, hearing loss, and fever. On examination, the pinna is pushed forward and downward, and the post-auricular groove is obliterated. The tympanic membrane cannot be adequately visualised. What is the most likely diagnosis?",
    "opts": [
      "Otitis externa",
      "Mastoiditis",
      "Cholesteatoma",
      "Referred otalgia from dental caries"
    ],
    "ans": 1,
    "exp": "Mastoiditis is a complication of acute otitis media in which infection spreads from the middle ear to the mastoid air cells. The classic signs are post-auricular erythema, tenderness, and swelling causing the pinna to be displaced forward and downward with obliteration of the post-auricular crease. This represents a surgical emergency requiring urgent IV antibiotics and cortical mastoidectomy if the abscess is not resolving. Dhingra's ENT emphasises that a displaced pinna with post-auricular swelling following AOM is mastoiditis until proven otherwise. Otitis externa (A) causes pinna tenderness on movement and tragus tenderness without post-auricular swelling or pinna displacement. Cholesteatoma (C) has a gradual onset with foul-smelling discharge. Dental otalgia (D) does not cause pinna displacement.",
    "section": "ENT",
    "num": 78
  },
  {
    "q": "A 22-year-old woman presents with recurrent episodes of nasal obstruction, watery rhinorrhoea, sneezing attacks in the morning, and itching of the eyes and palate, especially in spring. Anterior rhinoscopy shows pale, oedematous, bluish-grey inferior turbinates. What is the first-line pharmacological treatment?",
    "opts": [
      "Systemic oral corticosteroids",
      "Intranasal corticosteroid spray",
      "Systemic antihistamine alone",
      "Ipratropium bromide nasal spray"
    ],
    "ans": 1,
    "exp": "The clinical features of seasonal nasal obstruction, watery rhinorrhoea, sneezing, nasal and ocular pruritus, with pale bluish-grey oedematous inferior turbinates (classical endoscopic appearance of allergic rhinitis) are diagnostic of allergic rhinitis. For persistent or moderate-severe allergic rhinitis, intranasal corticosteroid (INCS) sprays (such as mometasone furoate or fluticasone) are the most effective first-line pharmacological treatment, superior to antihistamines alone in reducing all nasal symptoms. They reduce mucosal eosinophilic inflammation. Systemic corticosteroids (A) are not appropriate for long-term management of allergic rhinitis. Antihistamines alone (C) are effective for itching and rhinorrhoea but less effective for nasal blockage. Ipratropium (D) targets rhinorrhoea but not obstruction or sneezing.",
    "section": "ENT",
    "num": 79
  },
  {
    "q": "A 55-year-old male smoker presents with progressive hoarseness of voice for 4 months. There is no dysphagia, odynophagia, or neck swelling. Indirect laryngoscopy reveals a white patch on the left vocal cord that does not wipe off. What is the most appropriate next step?",
    "opts": [
      "Reassurance and voice rest for 4 weeks",
      "Microlaryngoscopy and biopsy of the lesion",
      "Empirical antifungal therapy",
      "Course of antibiotics and review in 6 weeks"
    ],
    "ans": 1,
    "exp": "Progressive hoarseness lasting more than 3 weeks in a smoker with a white patch (leukoplakia) on the vocal cord that cannot be wiped off must be considered malignant until proven otherwise. Leukoplakia of the vocal cord can represent dysplasia or squamous cell carcinoma in situ. The standard management is microlaryngoscopy under general anaesthesia with biopsy of the lesion for histopathological diagnosis. Any delay in diagnosis risks missing early glottic carcinoma, which has an excellent prognosis when detected early. Reassurance (A) and antibiotics (D) are inappropriate in the absence of infective features. Antifungal therapy (C) is for oral or laryngeal candidiasis, which typically presents as white plaques in an immunocompromised patient and can be wiped off.",
    "section": "ENT",
    "num": 80
  },
  {
    "q": "A 6-year-old child is brought to a Lahore clinic with recurrent tonsillitis: 7 documented episodes in the past year. Parents report snoring, poor sleep, and daytime fatigue. What is the recommended management?",
    "opts": [
      "Long-term prophylactic oral penicillin",
      "Tonsillectomy",
      "Adenoidectomy alone",
      "Watchful waiting for another year"
    ],
    "ans": 1,
    "exp": "The Paradise criteria define the indications for tonsillectomy: 7 or more episodes in 1 year, or 5 or more per year for 2 consecutive years, or 3 or more per year for 3 consecutive years, each episode documented and requiring treatment. This child meets the absolute indication with 7 episodes in one year. The additional features of snoring, poor sleep quality, and daytime fatigue suggest obstructive sleep-disordered breathing from tonsillar hypertrophy, which is a further indication. Tonsillectomy is the definitive treatment. Long-term prophylactic penicillin (A) is not standard practice for recurrent tonsillitis. Adenoidectomy alone (C) addresses nasopharyngeal obstruction but not tonsillar pathology. Watchful waiting (D) is not appropriate when Paradise criteria are met.",
    "section": "ENT",
    "num": 81
  },
  {
    "q": "A 68-year-old man presents with bilateral, progressive, symmetric hearing loss predominantly affecting high frequencies, with difficulty understanding speech in noisy environments. Audiometry reveals a sloping sensorineural pattern worse at higher frequencies. He has no tinnitus, vertigo, or history of noise exposure. What is the most likely diagnosis?",
    "opts": [
      "Noise-induced hearing loss",
      "Presbycusis",
      "Ototoxic hearing loss",
      "Meniere's disease"
    ],
    "ans": 1,
    "exp": "Presbycusis (age-related hearing loss) is the most common cause of sensorineural hearing loss in the elderly. It is characterised by bilateral, symmetric, gradual high-frequency sensorineural hearing loss with a sloping audiogram pattern, beginning at higher frequencies (4-8 kHz) and progressing to speech frequencies (1-2 kHz) over time. Difficulty in understanding speech in noisy environments (the 'cocktail party effect') is a hallmark feature. The pathology involves degeneration of cochlear hair cells (primarily outer hair cells in the basal turn) and spiral ganglion neurons. Noise-induced hearing loss (A) shows a characteristic 4 kHz notch and has an occupational history. Ototoxic loss (C) requires relevant drug history. Meniere's disease (D) causes episodic vertigo with fluctuating low-frequency loss and tinnitus.",
    "section": "ENT",
    "num": 82
  },
  {
    "q": "A 65-year-old man presents to an eye clinic in Peshawar with gradual painless loss of vision in both eyes over 2 years, worse at night and in dim light. Fundoscopy reveals waxy pale disc, attenuated arterioles, and bone-spicule pigmentation in the mid-periphery of the retina. What is the most likely diagnosis?",
    "opts": [
      "Age-related macular degeneration",
      "Retinitis pigmentosa",
      "Diabetic retinopathy",
      "Central retinal artery occlusion"
    ],
    "ans": 1,
    "exp": "The triad of progressive bilateral visual loss worse at night (nyctalopia), waxy pale optic disc, attenuated arterioles, and bone-spicule pigmentation in the mid-peripheral retina is the pathognomonic picture of retinitis pigmentosa (RP). RP is a group of inherited retinal dystrophies characterised by progressive degeneration of rod photoreceptors (causing nyctalopia and peripheral visual field loss) followed by cone involvement (central vision loss). Parsons' Diseases of the Eye describes bone-spicule pigmentation as the characteristic fundoscopic hallmark. AMD (A) affects the macula causing central vision loss with drusen and pigmentary changes at the posterior pole, not the periphery. Diabetic retinopathy (C) shows dot haemorrhages, hard exudates, and neovascularisation. CRAO (D) causes sudden monocular visual loss with a 'cherry red spot.'",
    "section": "Ophthalmology",
    "num": 83
  },
  {
    "q": "A 55-year-old diabetic patient presents with sudden painless profound loss of vision in his right eye. He has no light perception. On fundoscopy, the retina appears 'milky white' with a cherry red spot at the fovea. What is the diagnosis?",
    "opts": [
      "Central retinal vein occlusion",
      "Vitreous haemorrhage",
      "Central retinal artery occlusion",
      "Rhegmatogenous retinal detachment"
    ],
    "ans": 2,
    "exp": "Central retinal artery occlusion (CRAO) presents with sudden, profound, painless monocular visual loss, often with no light perception. The fundoscopic appearance of diffuse retinal oedema creating a milky-white opacification with a cherry-red spot at the fovea is pathognomonic. The cherry-red spot occurs because the fovea receives its oxygen supply from the underlying choroid (not the retinal artery), remaining perfused and appearing red against the white ischaemic retina. CRAO is an ocular emergency analogous to a stroke. Central retinal vein occlusion (A) shows a 'stormy sunset' fundus with flame haemorrhages in all four quadrants. Vitreous haemorrhage (B) blocks fundal view completely. Retinal detachment (D) shows a grey, undulating retina.",
    "section": "Ophthalmology",
    "num": 84
  },
  {
    "q": "A 70-year-old woman presents with a painless progressive decrease in central vision in her right eye, distortion of straight lines (metamorphopsia), and difficulty reading. Fundoscopy shows drusen and subretinal haemorrhage at the macula. What is the most likely diagnosis?",
    "opts": [
      "Glaucoma",
      "Age-related macular degeneration (wet type)",
      "Diabetic macular oedema",
      "Central serous chorioretinopathy"
    ],
    "ans": 1,
    "exp": "Age-related macular degeneration (AMD) is the leading cause of irreversible central visual loss in patients over 65 years. The wet (neovascular) type is characterised by choroidal neovascularisation beneath the retinal pigment epithelium, leading to subretinal haemorrhage and exudation. Clinically, patients present with rapid deterioration of central vision, metamorphopsia (distortion of straight lines due to disruption of macular photoreceptors by fluid), and difficulty reading. Drusen (extracellular deposits beneath the RPE) are the hallmark of AMD. Treatment for wet AMD is intravitreal anti-VEGF injection (ranibizumab, bevacizumab, aflibercept). Glaucoma (A) causes peripheral field loss with an optic nerve cupping. Diabetic macular oedema (C) requires diabetic history. Central serous chorioretinopathy (D) affects younger men with a stress history.",
    "section": "Ophthalmology",
    "num": 85
  },
  {
    "q": "A 35-year-old man with poorly controlled type 1 diabetes presents with floaters, reduced vision, and sees a 'red haze.' Fundoscopy is difficult due to hazy media. B-scan ultrasound confirms haemorrhage in the vitreous cavity. What is the most likely underlying cause?",
    "opts": [
      "Posterior vitreous detachment",
      "Proliferative diabetic retinopathy with vitreous haemorrhage",
      "Terson syndrome",
      "Sickle cell retinopathy"
    ],
    "ans": 1,
    "exp": "Vitreous haemorrhage in a young patient with poorly controlled type 1 diabetes and a history consistent with diabetic eye disease is most likely due to proliferative diabetic retinopathy (PDR). In PDR, retinal ischaemia drives neovascularisation on the retinal surface and posterior vitreous. These fragile new vessels (neovascularisation of the disc or elsewhere) rupture and bleed into the vitreous cavity, causing sudden floaters, reduced vision, and a red haze. This is a sight-threatening complication requiring urgent vitreoretinal assessment. Posterior vitreous detachment (A) can cause a vitreous haemorrhage but is less common in this age group and without diabetes. Terson syndrome (C) is vitreous haemorrhage secondary to subarachnoid haemorrhage. Sickle cell (D) requires relevant haematological history.",
    "section": "Ophthalmology",
    "num": 86
  },
  {
    "q": "A neonate born at term presents with a white reflex (leucocoria) in the left eye noticed by the mother on photographs. The child appears otherwise well. What is the most important diagnosis to exclude?",
    "opts": [
      "Congenital cataract",
      "Retinoblastoma",
      "Coats disease",
      "Persistent fetal vasculature"
    ],
    "ans": 1,
    "exp": "Leucocoria (white pupillary reflex) in an infant must be considered retinoblastoma until proven otherwise, as it is the most common primary intraocular malignancy of childhood and a life-threatening diagnosis. Early diagnosis is critical because retinoblastoma can metastasise via the optic nerve to the brain and via the blood to bone marrow and other organs. Any child presenting with leucocoria must have urgent ophthalmological assessment including dilated fundoscopy under general anaesthesia if necessary. While congenital cataract (A), Coats disease (C), and persistent fetal vasculature (D) can also cause leucocoria, the priority is to exclude retinoblastoma given its malignant nature and the consequences of delayed diagnosis. Parsons' Diseases of the Eye identifies leucocoria as a red flag sign requiring urgent assessment.",
    "section": "Ophthalmology",
    "num": 87
  },
  {
    "q": "A 50-year-old man presents with dull aching right eye pain, blurred vision, halos around lights particularly at night, and mild headache. His IOP is 28 mmHg in the right eye and 16 mmHg in the left. Gonioscopy shows open anterior chamber angles bilaterally. Visual field testing reveals an arcuate scotoma in the right eye. What is the most likely diagnosis?",
    "opts": [
      "Acute angle-closure glaucoma",
      "Primary open-angle glaucoma",
      "Ocular hypertension without glaucoma",
      "Normal-tension glaucoma"
    ],
    "ans": 1,
    "exp": "Primary open-angle glaucoma (POAG) is characterised by chronically elevated IOP with open drainage angles on gonioscopy, progressive optic nerve damage with characteristic cupping (increased cup-to-disc ratio), and corresponding visual field defects (arcuate scotoma, nasal step, tunnel vision in advanced disease). Halos around lights can occur due to corneal oedema from raised IOP. POAG is typically asymptomatic until late stages, though this patient has symptoms suggesting moderately advanced disease. Acute angle-closure glaucoma (A) presents with severe pain, nausea, corneal oedema, and a fixed mid-dilated pupil on a closed angle. Ocular hypertension (C) has elevated IOP without optic nerve damage or field loss. Normal-tension glaucoma (D) has glaucomatous changes with IOP within normal range.",
    "section": "Ophthalmology",
    "num": 88
  },
  {
    "q": "A 25-year-old medical student notices that her distance vision has become progressively blurred over the past year, but near vision remains excellent. Retinoscopy under cycloplegia shows a refractive error of -3.5 dioptres in both eyes. What is the correct type of lens for her spectacle correction?",
    "opts": [
      "Convex (converging) lens",
      "Concave (diverging) lens",
      "Cylindrical lens",
      "Bifocal lens"
    ],
    "ans": 1,
    "exp": "The clinical picture of blurred distance vision with preserved near vision, and a negative (minus) refractive error on retinoscopy, confirms myopia (short-sightedness). In myopia, the parallel light rays from distant objects are focused anterior to the retina because the eyeball is relatively too long or the lens too powerful. Correction requires a concave (diverging) lens, which diverges the incoming light before it enters the eye, moving the focal point back onto the retina. The prescription is expressed as a negative dioptre value. A convex (converging) lens (A) is used for hypermetropia (long-sightedness) and presbyopia. Cylindrical lenses (C) correct astigmatism. Bifocals (D) are used for presbyopia requiring both distance and near correction.",
    "section": "Ophthalmology",
    "num": 89
  },
  {
    "q": "A 55-year-old male rickshaw driver from Lahore presents with a 10-year history of productive cough on most days for at least 3 months per year, progressive exertional dyspnoea, and a 30-pack-year smoking history. Spirometry shows FEV1/FVC ratio of 0.58 post-bronchodilator. What is the most likely diagnosis?",
    "opts": [
      "Bronchial asthma",
      "Pulmonary tuberculosis",
      "Chronic obstructive pulmonary disease (COPD)",
      "Bronchiectasis"
    ],
    "ans": 2,
    "exp": "The combination of chronic productive cough (at least 3 months per year for 2 consecutive years — the definition of chronic bronchitis), progressive exertional dyspnoea, a significant smoking history, and post-bronchodilator FEV1/FVC ratio below 0.70 confirms a diagnosis of COPD. COPD encompasses chronic bronchitis and emphysema, the two pathological processes driven by tobacco smoke. Post-bronchodilator spirometry is the gold standard for confirming irreversible airflow obstruction. Asthma (A) is typically reversible, variable, often with an atopic history, and the FEV1/FVC ratio normalises post-bronchodilator. TB (B) would show constitutional symptoms, haemoptysis, and CXR infiltrates. Bronchiectasis (D) presents with copious purulent sputum, and CT chest shows dilated, thickened bronchi.",
    "section": "Pulmonology",
    "num": 90
  },
  {
    "q": "A 30-year-old woman presents with a 3-month history of low-grade fever, night sweats, weight loss, and a productive cough with occasional blood-stained sputum. She lives in a crowded urban settlement in Rawalpindi. CXR shows upper lobe infiltrates with cavitation. Sputum AFB smear is positive. What is the standard initial treatment regimen?",
    "opts": [
      "Two months of isoniazid and rifampicin, followed by four months of isoniazid alone",
      "Six months of isoniazid, rifampicin, pyrazinamide, and ethambutol throughout",
      "Two months of isoniazid, rifampicin, pyrazinamide, and ethambutol, followed by four months of isoniazid and rifampicin",
      "Three months of isoniazid, rifampicin, and streptomycin"
    ],
    "ans": 2,
    "exp": "Pulmonary tuberculosis is treated with the WHO-recommended standard short-course regimen: an intensive phase of 2 months with four drugs (isoniazid, rifampicin, pyrazinamide, ethambutol — abbreviated 2HRZE) followed by a continuation phase of 4 months with two drugs (isoniazid and rifampicin — 4HR). This 2HRZE/4HR regimen is used in Pakistan's National TB Control Programme. The four-drug intensive phase ensures rapid bacillary killing and prevents the emergence of drug resistance. Pyrazinamide and ethambutol are particularly important in killing persistent intracellular organisms and preventing resistance. Option A is insufficient. Option B is unnecessarily prolonged with four drugs. Option D includes streptomycin which is not part of first-line standard regimens.",
    "section": "Pulmonology",
    "num": 91
  },
  {
    "q": "A 32-year-old woman with known asthma is admitted with acute severe asthma. She is tachypnoeic at 32 breaths/minute, cannot complete a sentence, has widespread bilateral wheeze, and her PEFR is 38% of predicted. Oxygen saturations are 94% on air. She has received two nebulised salbutamol treatments. What is the next most important addition to her management?",
    "opts": [
      "Intravenous theophylline as the first priority",
      "Oral or intravenous corticosteroids",
      "Immediate intubation and mechanical ventilation",
      "Nebulised ipratropium bromide as monotherapy"
    ],
    "ans": 1,
    "exp": "In acute severe asthma (PEFR 33-50%, respiratory rate above 25, inability to complete sentences, heart rate above 110), management requires supplemental oxygen, repeated bronchodilators (salbutamol and ipratropium nebulisers), and systemic corticosteroids. Corticosteroids (oral prednisolone 40-50 mg or IV hydrocortisone) are essential to suppress the underlying airway inflammation and reduce the risk of deterioration and relapse. They take 4-6 hours to act. After adequate bronchodilator use, systemic corticosteroids represent the next critical intervention. IV theophylline (A) is a second-line add-on, not the next priority. Immediate intubation (C) is for life-threatening or near-fatal asthma. Ipratropium as monotherapy (D) is incorrect; it should be added to salbutamol, not used instead of it.",
    "section": "Pulmonology",
    "num": 92
  },
  {
    "q": "A 45-year-old man is found to have a 2 cm solitary pulmonary nodule on a chest X-ray done for unrelated reasons. He is a non-smoker with no symptoms, no weight loss, and no fever. The nodule has smooth margins and contains central calcification on CT chest. What is the most likely nature of this nodule?",
    "opts": [
      "Primary lung adenocarcinoma",
      "Metastatic deposit",
      "Benign granuloma (likely old TB or histoplasmosis)",
      "Carcinoid tumour"
    ],
    "ans": 2,
    "exp": "A solitary pulmonary nodule with central 'popcorn' or laminated calcification, smooth margins, and absence of symptoms or constitutional features in a non-smoker is most consistent with a benign granuloma, commonly from old healed tuberculosis (the most common cause in Pakistan and South Asia) or fungal infection. CT features strongly favouring benignity include: central, laminated, or popcorn calcification; smooth well-defined margins; small size below 6 mm; and stability over more than 2 years. Malignant nodules (A, B) typically have spiculated or lobulated margins, are non-calcified or have eccentric calcification, and occur in smokers. A carcinoid (D) is usually endobronchial and causes obstructive symptoms.",
    "section": "Pulmonology",
    "num": 93
  },
  {
    "q": "A 40-year-old woman is admitted to a Karachi hospital with sudden onset pleuritic chest pain, dyspnoea, and haemoptysis 5 days after a total abdominal hysterectomy. Her heart rate is 118/min and oxygen saturation is 90% on room air. D-dimer is markedly elevated. CT pulmonary angiography confirms bilateral pulmonary emboli. She has no contraindications to anticoagulation. What is the most appropriate initial treatment?",
    "opts": [
      "Subcutaneous low-molecular-weight heparin followed by oral anticoagulation",
      "Thrombolytic therapy with alteplase as first-line",
      "Inferior vena cava filter insertion alone",
      "Aspirin and observation"
    ],
    "ans": 0,
    "exp": "Pulmonary embolism (PE) confirmed on CTPA in a haemodynamically stable patient (no hypotension, no cardiac arrest) is managed with therapeutic anticoagulation. Low-molecular-weight heparin (LMWH) is given initially as a bridging therapy, followed by transition to a DOAC (rivaroxaban or apixaban) or vitamin K antagonist. Recent post-operative status is a temporary contraindication to DOACs initially, making LMWH the preferred initial agent. Thrombolysis (B) is reserved for massive PE with haemodynamic compromise (hypotension, cardiac arrest). This patient is tachycardic and hypoxic but haemodynamically stable, so thrombolysis is not first-line. IVC filter (C) is for patients with contraindications to anticoagulation. Aspirin (D) is inadequate for PE treatment.",
    "section": "Pulmonology",
    "num": 94
  },
  {
    "q": "A 28-year-old man presents with dry cough, fever, bilateral hilar lymphadenopathy on CXR, and erythema nodosum on his shins. Serum ACE level is elevated. Bronchoscopy with BAL shows lymphocytosis with CD4:CD8 ratio of 5:1. What is the most likely diagnosis?",
    "opts": [
      "Pulmonary tuberculosis",
      "Sarcoidosis",
      "Lymphoma with pulmonary involvement",
      "Hypersensitivity pneumonitis"
    ],
    "ans": 1,
    "exp": "The combination of bilateral hilar lymphadenopathy (BHL), erythema nodosum, elevated serum ACE, and BAL lymphocytosis with a high CD4:CD8 ratio is the classic presentation of sarcoidosis. Lofgren's syndrome (BHL + erythema nodosum + arthralgia + fever) is a specific presentation with a good prognosis. Sarcoidosis is a multisystem granulomatous disease of unknown aetiology characterised by non-caseating granulomas. Elevated ACE (produced by granuloma macrophages) is a useful but non-specific marker. TB (A) would show caseating granulomas, AFB positivity, and a different clinical pattern. Lymphoma (C) typically has mediastinal involvement and systemic B symptoms. Hypersensitivity pneumonitis (D) follows antigen exposure and shows CD8 lymphocytosis in BAL. Davidson's Medicine identifies this constellation as characteristic of sarcoidosis.",
    "section": "Pulmonology",
    "num": 95
  },
  {
    "q": "A 28-year-old woman is brought to a psychiatry clinic by her family. They report that for the past 3 weeks she has been sleeping only 2-3 hours per night, spending large sums of money on unnecessary items, speaking very rapidly, and believing she has been chosen by God to lead the country. On examination, she is distractible, euphoric, and shows flight of ideas. She has no prior psychiatric history. What is the most likely diagnosis?",
    "opts": [
      "Schizophrenia",
      "Manic episode",
      "Substance-induced mood disorder",
      "Borderline personality disorder"
    ],
    "ans": 1,
    "exp": "The clinical picture of reduced sleep without tiredness, pressured speech, flight of ideas, grandiose delusions (chosen by God to lead), distractibility, and elevated mood lasting 3 weeks is the prototypical presentation of a manic episode. A manic episode requires elevated or irritable mood with at least three of: increased goal-directed activity, decreased need for sleep, pressured speech, grandiosity, flight of ideas, distractibility, and excessive involvement in pleasurable high-risk activities, lasting at least 1 week and causing marked functional impairment. Schizophrenia (A) would typically show negative symptoms, flat affect, and hallucinations without this degree of mood elevation. Substance-induced disorder (C) requires a relevant history. Borderline personality disorder (D) shows impulsivity and emotional dysregulation but not sustained manic episodes.",
    "section": "Psychiatry",
    "num": 96
  },
  {
    "q": "A 35-year-old man presents to a Lahore mental health clinic with a 2-year history of persistent low mood, anhedonia, insomnia, fatigue, poor concentration, and feelings of worthlessness. He has not had any episodes of elevated mood. Hamilton Depression Rating Scale score is 26. No organic cause is found. What is the first-line pharmacological treatment?",
    "opts": [
      "Tricyclic antidepressant (amitriptyline)",
      "Selective serotonin reuptake inhibitor (SSRI)",
      "Monoamine oxidase inhibitor (MAOI)",
      "Benzodiazepine"
    ],
    "ans": 1,
    "exp": "Major depressive disorder (MDD) with persistent low mood, anhedonia, vegetative symptoms (insomnia, fatigue), cognitive symptoms (poor concentration), and feelings of worthlessness of 2 years duration without manic episodes is treated first-line with an SSRI (e.g., fluoxetine, sertraline, escitalopram). SSRIs are preferred due to their favourable side effect profile, safety in overdose, and established efficacy for moderate-to-severe depression. The Shorter Oxford Textbook of Psychiatry recommends SSRIs as first-line pharmacological treatment for depressive disorder. Tricyclic antidepressants (A) are effective but have significant anticholinergic side effects, cardiotoxicity in overdose, and a narrow therapeutic window. MAOIs (C) require dietary tyramine restriction and have multiple interactions. Benzodiazepines (D) are not antidepressants.",
    "section": "Psychiatry",
    "num": 97
  },
  {
    "q": "A 60-year-old man is brought to a psychiatric unit by his family with a 6-month gradual decline in memory, particularly forgetting recent events while remote memory is relatively preserved, getting lost in familiar surroundings, difficulty with daily tasks, and personality changes. His MMSE score is 19/30. CT brain shows generalised cortical atrophy with hippocampal volume loss. What is the most likely diagnosis?",
    "opts": [
      "Vascular dementia",
      "Alzheimer's dementia",
      "Frontotemporal dementia",
      "Delirium"
    ],
    "ans": 1,
    "exp": "Alzheimer's dementia (AD) is the most common cause of dementia worldwide and in Pakistan. It presents insidiously with progressive anterograde amnesia (inability to form new memories) while remote memory is relatively preserved early on. Patients develop difficulty with orientation (getting lost in familiar environments), instrumental activities of daily living, and personality and behavioural changes. CT/MRI shows generalised atrophy with disproportionate hippocampal atrophy. MMSE below 24 indicates cognitive impairment. The Shorter Oxford Textbook of Psychiatry identifies this gradual, memory-first, hippocampal-predominant pattern as characteristic of AD. Vascular dementia (A) has a stepwise decline with a history of cerebrovascular events and periventricular white matter changes. Frontotemporal dementia (C) presents with personality and behavioural changes preceding memory loss. Delirium (D) is acute, fluctuating, and has an identifiable cause.",
    "section": "Psychiatry",
    "num": 98
  },
  {
    "q": "A 19-year-old woman is referred to a psychiatry clinic with an 18-month history of recurrent episodes of consuming extremely large quantities of food in a short period, followed by self-induced vomiting to compensate. She feels ashamed and hides this behaviour. Her BMI is 21 kg/m². Physical examination reveals erosion of dental enamel and calluses on the dorsum of her right hand. What is the most likely diagnosis?",
    "opts": [
      "Anorexia nervosa, binge-purge subtype",
      "Bulimia nervosa",
      "Binge eating disorder",
      "Avoidant restrictive food intake disorder"
    ],
    "ans": 1,
    "exp": "Bulimia nervosa is characterised by recurrent episodes of binge eating (consuming abnormally large amounts of food with loss of control), followed by compensatory behaviours (self-induced vomiting, laxative use, excessive exercise) at least once per week for 3 months, and self-evaluation disproportionately influenced by body shape and weight. The BMI is typically normal or even above normal, distinguishing it from anorexia nervosa. Physical signs include Russell's sign (calluses on knuckles from repeated self-induced vomiting), dental enamel erosion from gastric acid, and parotid gland hypertrophy. Anorexia nervosa binge-purge subtype (A) would have a BMI below 17.5 kg/m². Binge eating disorder (C) has no compensatory behaviour. ARFID (D) involves restricted food variety without bingeing.",
    "section": "Psychiatry",
    "num": 99
  },
  {
    "q": "A 30-year-old man with chronic schizophrenia has been on haloperidol for 3 years. He develops involuntary, repetitive, choreiform movements of the tongue, lips, and face, along with writhing movements of the fingers, that persist even when the antipsychotic dose is reduced. What is this adverse effect called?",
    "opts": [
      "Acute dystonia",
      "Akathisia",
      "Tardive dyskinesia",
      "Neuroleptic malignant syndrome"
    ],
    "ans": 2,
    "exp": "Tardive dyskinesia (TD) is a late-onset movement disorder caused by prolonged exposure to dopamine receptor-blocking agents, particularly first-generation (typical) antipsychotics such as haloperidol. It is characterised by involuntary, repetitive, stereotyped movements predominantly affecting the orofacial region (tongue protrusion, lip smacking, chewing), trunk, and extremities (piano-playing finger movements). Crucially, TD persists or worsens on dose reduction (as dopamine receptors become transiently more supersensitive), distinguishing it from acute extrapyramidal effects. Acute dystonia (A) occurs within days of starting treatment: sustained muscle contractions causing abnormal postures. Akathisia (B) is a subjective sense of inner restlessness and compulsion to move. Neuroleptic malignant syndrome (D) presents with hyperthermia, rigidity, autonomic instability, and raised CK, and is a medical emergency.",
    "section": "Psychiatry",
    "num": 100
  }
];
